...
ina226_err_t ina226_initialize(ina226_t *ina226, ina226_config_t const *config,
...
ina226_err_t ina226_get_current_scaled(ina226_t const *ina226,
                                       float32_t *scaled) {
...
ina226_err_t ina226_get_bus_voltage_scaled(ina226_t const *ina226,
                                           float32_t *scaled) {
  assert(ina226 && scaled);
...
ina226_err_t ina226_get_shunt_voltage_scaled(ina226_t const *ina226,
                                             float32_t *scaled) {
  assert(ina226 && scaled);
...
ina226_err_t ina226_get_power_scaled(ina226_t const *ina226,
                                     float32_t *scaled) {
  assert(ina226 && scaled);
  ...
  *scaled = (float32_t)raw *
            ina226_current_to_power_scale(ina226->config.current_scale);
...
ina226_err_t ina226_get_shunt_voltage_raw(ina226_t const *ina226,
                                          int16_t *raw) {
  assert(ina226 && raw);
...
ina226_err_t ina226_get_config_reg(ina226_t const *ina226,
                                   ina226_config_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(ina226, INA226_REG_ADDRESS_CONFIG,
                                          data, sizeof(data));
...
ina226_err_t ina226_set_config_reg(ina226_t const *ina226,
                                   ina226_config_reg_t const *reg) {
  assert(ina226 && reg);
  ...
  err |= ina226_bus_write_data(ina226, INA226_REG_ADDRESS_CONFIG, data,
                               sizeof(data));

  return err;
}

ina226_err_t ina226_get_shunt_voltage_reg(ina226_t const *ina226,
                                          ina226_shunt_voltage_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_SHUNT_VOLTAGE, data, sizeof(data));
...
ina226_err_t ina226_get_bus_voltage_reg(ina226_t const *ina226,
                                        ina226_bus_voltage_reg_t *reg) {
  assert(ina226 && reg);
  ...
  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_BUS_VOLTAGE, data, sizeof(data));
...
ina226_err_t ina226_get_power_reg(ina226_t const *ina226,
                                  ina226_power_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(ina226, INA226_REG_ADDRESS_POWER,
                                          data, sizeof(data));
...
ina226_err_t ina226_get_current_reg(ina226_t const *ina226,
                                    ina226_current_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(ina226, INA226_REG_ADDRESS_CURRENT,
                                          data, sizeof(data));
...
ina226_err_t ina226_get_calibration_reg(ina226_t const *ina226,
                                        ina226_calibration_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_CALIBRATION, data, sizeof(data));
...
ina226_err_t ina226_get_mask_enable_reg(ina226_t const *ina226,
                                        ina226_mask_enable_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_MASK_ENABLE, data, sizeof(data));
...
ina226_err_t ina226_set_mask_enable_reg(ina226_t const *ina226,
  ...
  data[0] &= ~((0x01U << 7U) | (0x01U << 6U) | (0x01U << 5U) | (0x01U << 4U) |
               (0x01U << 3U) | (0x01U << 2U));
  data[1] &=
      ~((0x01 << 4U) | (0x01 << 3U) | (0x01 << 2U) | (0x01 << 1U) | 0x01);
  ...
  err |= ina226_bus_write_data(ina226, INA226_REG_ADDRESS_MASK_ENABLE, data,
                               sizeof(data));

  return err;
}

ina226_err_t ina226_get_alert_limit_reg(ina226_t const *ina226,
                                        ina226_alert_limit_reg_t *reg) {
  assert(ina226 && reg);

  uint8_t data[2] = {};

  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_ALERT_LIMIT, data, sizeof(data));
...
ina226_err_t ina226_set_alert_limit_reg(ina226_t const *ina226,
  ...
  return ina226_bus_write_data(ina226, INA226_REG_ADDRESS_ALERT_LIMIT, data,
                               sizeof(data));
}
...
ina226_err_t ina226_get_manufacturer_id_reg(ina226_t const *ina226,
  ...
  ina226_err_t err = ina226_bus_read_data(
      ina226, INA226_REG_ADDRESS_MANUFACTURER_ID, data, sizeof(data));
...
ina226_err_t ina226_get_die_id_reg(ina226_t const *ina226,
  ...
  ina226_err_t err = ina226_bus_read_data(ina226, INA226_REG_ADDRESS_DIE_ID,
                                          data, sizeof(data));
